...
> **Our goal:** <br> Develop an method to measure media slant based on the visual reporting of media outlets on specific events.


## What We Do and What We Find

**We propose a two-step, unsupervised framework for scaling images:**

1. Encode images with **Vision Transformers (ViT**) → analyse pairwise image similarity
2. Recover latent positions with **Additive-Multiplicative Effects** (AME) network models

. . .

<br>

**Key findings** (Gaza War, 10 outlets, Oct 2023--Oct 2024):

- Far-right outlets (Fox News, Breitbart) cluster visually; Al Jazeera English occupies the opposite pole
- The latent dimension separates *political-elite framing* from *on-the-ground depictions of suffering*
...
1. Encode each image with a pre-trained **Vision Transformer (ViT)**
   <br> → extract the `[CLS]` token embedding (512 dims)
2. Compute pairwise **cosine similarities** across all image dyads
3. Estimate variation of the Additive-Multiplicative Effects (AME) network model to recover latent positions of
...
$$\mathrm{atanh}\!\left(\mathrm{sim}(\vec{E}_i, \vec{E}_j)\right) = \textcolor{blue}{\mu} + \textcolor{red}{\vec{S}_i^\top \vec{S}_j} + \epsilon_{ij}, \quad \epsilon_{ij} \sim \mathcal{N}(0,\sigma^2)$$

<br>

. . .

[$\mu$]{style="color: blue"} **Global intercept**: Average baseline similarity across all image pairs
...
- **Out-of-sample projection** scales the approach to millions of images
...
- 10 cross-national news outlets:
...
```{r image-level-1, height = 6, width = 9, cache=T}
...
## Dynamic Visual Slant: Stable Sorting, Parallel Dynamics

```{r img-time-1, width = 14, height = 8, cache=T}
...
```{r img-time-2, width = 14, height = 8, cache=T}
...
```{r img-time-3, width = 14, height = 8, cache=T}
...
```{r img-time-4, width = 14, height = 8, cache=T}
...
- First unsupervised, scalable framework for recovering latent positions from visual corpora
- Extends the quantitative toolkit of political science from text to images
...
- Analyze over time dynamics


## {background-color="#1d3557"}

::: {style="color: white; text-align: center; margin-top: 30%;"}
